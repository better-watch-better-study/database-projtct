...
class Monitoring_Information_DAO_Impl(Base_DAO, Monitoring_Information_DAO):
    ...
    def select(self, mes_to_select):
        ...
        cursor = self.connection.cursor()
        cursor.execute("select mno, MI.mpno, Plants_name, mdno, mtime, mplace, ctime, cperson, temperature, humidity, cbc, disease, light from MI inner join basicPlants on MI.mpno=basicPlants.Plants_number where " + k + "=" + "'" + v + "'")
    ...
    def show(self):
        cursor = self.connection.cursor()
        cursor.execute("select mno, MI.mpno, Plants_name, mdno, mtime, mplace, ctime, cperson, temperature, humidity, cbc, disease, light from MI inner join basicPlants on MI.mpno=basicPlants.Plants_number")
        result = cursor.fetchall()
        print("监测编号 监测植物编号 监测植物名称 监测设备编号 监测时间 监测地点 创建时间 创建人员 温度 湿度 二氧化碳浓度 病虫害信息 光照强度")
        for res in result:
            lres = list(res)
